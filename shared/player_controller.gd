...
@onready var _character_skin: MiniSophiaSkin = $RotationRoot/CharacterSkin
...
func _physics_process(delta: float) -> void:
	...
	if _ground_raycast.is_colliding():
		...
		_ground_height = global_position.y + _ground_raycast.target_position.y
...
func _get_camera_oriented_input() -> Vector3:
	...
	input = get_viewport().get_camera_3d().global_transform.basis * input
	input.y = 0.0
